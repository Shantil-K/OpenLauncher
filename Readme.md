...
- **Apps**: size of the bottom bar and of the app menu, **Hide pinned apps** (off by default: leaves the apps that are on the bottom bar out of the app menu so each shows once; Edit mode still lists them), **System apps** (off by default: the TV marks its inputs, services and helper apps as not meant for an app list, 134 of 189 on an LG C2, and the app menu leaves them out; Live TV, the HDMI inputs and Settings stay on the bottom bar either way), how dark the app menu's **Background** is (a 0 to 100% slider, 50% by default; the clock, date and weather are always hidden while the menu is open), names under the apps and their size (bar and menu separately; the size only shows while names are on), a **Recent apps** row above the bottom bar (off by default), **Folders** (add, rename, delete), and the colour used when hovering an app (cyan by default).
...
- **Reorder icon (two arrows)**: hover-only, no clicking. Rest the pointer on it for a little under a second (it fills
  ...
  **This is also how you pin:** carry an app from the app menu over the bottom bar and it is pinned there (in the place of the tile
  you are over, or at the end if you hover the empty part of the bar; a hidden app is shown again). Pinning is a one-way trip: an app you carry
  in the bar can be reordered along the bar, but carrying it back over the app menu does nothing. The only way to take an app off the bar is
  the **Unpin** button of its tile in Edit mode (or "Unpin from the bar" in its OK menu). Pinned apps have a ring in the hover colour in the app menu.
...
- **Unpin icon (on the bottom bar's tiles only)**: the only way to take an app off the bottom bar. (The app menu has no pin buttons; you pin by carrying an app onto the bar.)
...
- In **Edit mode**, OK on an app opens a small menu: hide/show, move to folder, **Rename...**, **Reorder** (then the arrows move
  the app, and OK or Back drops it; **Down** past the last row of the menu pins it to the bottom bar; it does not go back, use **Unpin from the bar** in the bar tile's OK menu) and **Uninstall...** (asks first; the focus starts on Cancel).
